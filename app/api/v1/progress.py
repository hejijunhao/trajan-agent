...
# =============================================================================
# Dashboard Progress Endpoints (Cross-Product Aggregation)
# =============================================================================
...
async def get_dashboard_progress(
    ...
    all_products: list[tuple[Any, Any]] = []  # (product, org_id)
    ...
    daily_activity = _generate_daily_activity(dict(aggregate_stats["daily_activity"]), period)
...
async def generate_dashboard_progress(
    ...
    This is a synchronous operation that may take several seconds depending
    on the number of products. For large portfolios, consider adding
    background job support in the future.
    """
    from app.domain import dashboard_shipped_ops, org_member_ops, product_ops
                ...
                for commit in commits:
                    ...
                    product_commits.append(
                        CommitInfo(
                            ...
                            timestamp=timestamp,
                            files=[],  # We don't fetch file details for performance
                        )
                    )
