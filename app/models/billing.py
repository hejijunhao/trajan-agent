...
class ReferralStatus(str, Enum):
    ...
    PENDING = "pending"  # Referral link shared, no signup yet
    SIGNED_UP = "signed_up"  # Referee created account
    CONVERTED = "converted"  # Referee upgraded to paid plan
    CREDITED = "credited"  # Referrer received credit
...
class Referral(SQLModel, table=True):
    ...
    Each referral tracks one potential conversion from referrer to referee.
    """
    ...
    referrer_org_id: uuid_pkg.UUID = Field(
        sa_column=Column(
            PG_UUID(as_uuid=True),
            ForeignKey("organizations.id", ondelete="CASCADE"),
            nullable=False,
    ...
    referrer_user_id: uuid_pkg.UUID = Field(
        sa_column=Column(
            PG_UUID(as_uuid=True),
            ForeignKey("users.id", ondelete="SET NULL"),
            nullable=True,
    ...
    referee_org_id: uuid_pkg.UUID | None = Field(
        default=None,
        sa_column=Column(
            PG_UUID(as_uuid=True),
    ...
    referee_email: str | None = Field(
        default=None,
        max_length=255,
        nullable=True,
        sa_column_kwargs={"comment": "Email captured before signup"},
    )
    ...
    # Credit amount (in cents) - default $100
    ...
    referrer_org: Optional["Organization"] = Relationship(
        sa_relationship_kwargs={"foreign_keys": "[Referral.referrer_org_id]"}
    )
    referrer_user: Optional["User"] = Relationship(
        sa_relationship_kwargs={"foreign_keys": "[Referral.referrer_user_id]"}
    )
    referee_org: Optional["Organization"] = Relationship(
        sa_relationship_kwargs={"foreign_keys": "[Referral.referee_org_id]"}
    )
