...
class TestOrgMemberCRUD:
    ...
    async def test_add_member(self, db_session: AsyncSession, test_org, second_user, test_user):
    ...
    async def test_update_role(self, db_session: AsyncSession, test_org_member):
    ...
    async def test_remove_member(
        ...
        removed = await org_member_ops.remove_member(db_session, test_org.id, second_user.id)
        assert removed is True

        # Verify they're gone
        member = await org_member_ops.get_by_org_and_user(db_session, test_org.id, second_user.id)
        assert member is None

    async def test_remove_nonexistent_member(self, db_session: AsyncSession, test_org, second_user):
...
class TestOrgMemberOwnership:
    ...
    async def test_is_only_owner_true(self, db_session: AsyncSession, test_org, test_user):
        """Returns True when user is the only owner."""
        assert await org_member_ops.is_only_owner(db_session, test_org.id, test_user.id) is True
    ...
    async def test_is_only_owner_false_after_transfer(
        ...
        assert await org_member_ops.is_only_owner(db_session, test_org.id, test_user.id) is False

    async def test_get_owners(self, db_session: AsyncSession, test_org, test_user):
